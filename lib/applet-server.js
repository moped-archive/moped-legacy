...
exports.run = function run(options) {
  ...
  function handlePost(req, res, next) {
    if (applet.postHandlers[req.path.substr(1)]) {
      parseJson(req, res, function (err) {
        if (err) return next(err);
        Promise.resolve(null).then(function () {
          return applet.postHandlers.apply(req, req.body);
  ...
  return function (req, res, next) {
    ...
    script.then(function (script) {
      ...
      applet.handleFirst(new Request(req.url, req.user, state)).then(function (result) {
