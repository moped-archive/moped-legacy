...
var url = '/';
var state = element.getAttribute('data-state') ? JSON.parse(element.getAttribute('data-state')) : {};
var user = element.getAttribute('data-user') ? JSON.parse(element.getAttribute('data-user')) : null;

exports.refresh = function (notFound) {
  if (notFound === true) location.reload();
  if (this.parent) return this.parent.refresh(notFound);
  var result = this.handleRender(this.req.update(url, user, state), this.refresh.bind(this));
  if (!result) {
...
exports.run = function () {
  ...
  page('*', function (ctx, next) {
    ...
    if (first) {
      ...
      return this.handleInit(this.req, this.refresh.bind(this)).done(function (result) {
    ...
    return this.handleNavigate(this.req.update(url, user, state), this.refresh.bind(this)).done(function (result) {
      if (!result) {
        page.stop();
        window.location = ctx.canonicalPath;
        return;
